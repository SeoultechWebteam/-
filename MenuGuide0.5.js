...
['W', 2.1, 3.1, 4.1, '카레',"url(https://raw.githubusercontent.com/SeoultechWebteam/images/1e10ba7093d4b88f7371742fdc3ed2ccb19d1054/%EC%B9%B4%EB%A0%88.jpg)"], //양식시작
...
var QuestionResult = new Array(QueSize); //질문들의 결과를 저장할 배열, 배열의 크기=질문수
for(var i=0;i<QueSize;i++) {
	QuestionResult[i] = '';
}
var checkcount = new Array(QueSize); //질문당 인덱스 체크수
...
function getQnumVal(event, number) { //number=문제번호
	var resultObj = document.getElementsByName(event.target.name); //체크박스 객체 저장

	if(event.target.value == number) { //Skip 눌렀을 때
		for(var i=0;i<resultObj.length - 1;i++) {
			if(resultObj[i].checked == event.target.checked) checkcount[number-1]--; //이미같은 상태인건 체크횟수 제외
			resultObj[i].checked = event.target.checked; //모두 체크/해제
		}
	}

	if(event.target.checked) { //체크하면
		checkcount[number-1]++;
		QuestionResult[number-1] = new Array(checkcount[number-1]);//체크수만큼 2차원배열 선언
	}
	else { //체크해제
		checkcount[number-1]--;
		QuestionResult[number-1] = new Array(checkcount[number-1]);
	}
	var ii = 0; //질문결과의 인덱스 번호
	for(var j=0;j<resultObj.length;j++) {//질문당 총 인덱스 수
		if(resultObj[j].checked) {//질문당 체크된 인덱스에서
			QuestionResult[number-1][ii] = resultObj[j].value; //체크된 인덱스 value값 저장
			ii++;
		}
	}

	document.getElementById('Result').innerHTML = null; //출력창 초기화
	for(var i=0;i<QuestionResult.length;i++) { //모든 결과를 출력
		for(var j=0;j<QuestionResult[i].length;j++) {
			document.getElementById('Result').innerHTML += QuestionResult[i][j] + ' ';
		}
...
function getMenu() {
	var pick = 0;
	var PickedMenu = new Array();
	var issame = false; //값이 동일한지 판별
	var theMENU = ''; //메뉴출력 초기화
	var themenuIMG = null; //이미지출력 초기화
	...
	for(var m=0;m<MenuArr.length;m++) { //m번째 데이터가
		for(var q=0;q<QuestionResult.length;q++) { //q번째 질문에서
			for(var i=0;i<QuestionResult[q].length;i++) { //한 질문에서 체크된 인덱스 수만큼 검색
				if(MenuArr[m][q]==QuestionResult[q][i]) { //데이터와 결과의 인덱스와 일치하는 것을 찾으면
					issame = true;
					break;
				}
				else { //일치하지 않으면
			...
			if(q==3 && issame == true) { //마지막 질문까지 검사해서 모든 데이터와 결과가 같다면
				PickedMenu[pick] = m; //데이터 번호를 저장
				pick++; //픽한 메뉴수를 늘리고
			}
	...
	for(var i=0;i<PickedMenu.length;i++) {
		theMENU += MenuArr[PickedMenu[i]][4] + '<br>'; //마지막에서 두번째는 메뉴이름
		themenuIMG = MenuArr[PickedMenu[i]][5]; //마지막은 이미지
	}
	if(theMENU == '') alert("메뉴 없음"); //추천메뉴가 없으면 추천을 안함
	document.getElementById('MENU').innerHTML = theMENU; //메뉴출력
	document.getElementById('menuIMG').style.backgroundImage = themenuIMG; //이미지 출력
}
...
function resetAll() {
	...
	for(var i=0;i<qEm.length;i++) {
		if(qEm[i].type == "checkbox") qEm[i].checked = false;
	}	
	//출력초기화
